...
static struct options opt = {
	...
	.o_fpath	  = {0},
	.o_fsname	  = {0},
	.o_storage_dest   = FSD_STORAGE_NULL
};
...
static void usage(const char *cmd_name, const int rc)
{
	fprintf(stdout, "usage: %s [options]\n"
		...
		DEFAULT_FPATH_NAME,
		FSD_STORAGE_DEST_STR(opt.o_storage_dest),
		PACKAGE_VERSION);
	...
	while ((c = getopt_long(argc, argv, "z:b:t:d:f:a:o:n:p:s:v:h",
		...
		case 'o': {
			if (OPTNCMP("null", optarg))
				opt.o_storage_dest = FSD_STORAGE_NULL;
			else if (OPTNCMP("local", optarg))
				opt.o_storage_dest = FSD_STORAGE_LOCAL;
			else if (OPTNCMP("lustre", optarg))
				opt.o_storage_dest = FSD_STORAGE_LUSTRE;
			else if (OPTNCMP("tsm", optarg))
				opt.o_storage_dest = FSD_STORAGE_TSM;
			else if (OPTNCMP("lustre_tsm", optarg))
				opt.o_storage_dest = FSD_STORAGE_LUSTRE_TSM;
			else {
...
static void *perform_task(void *thread_data)
{
	...
	while (next_idx < opt.o_nfiles) {
		...
		rc = fsd_fdopen(opt.o_fsname, fpath, NULL, opt.o_storage_dest,
				session);
