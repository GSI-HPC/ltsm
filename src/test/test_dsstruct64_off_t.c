...
void test_dsstruct64_off_t(CuTest *tc)
{
    off_t size_off_t;
    dsStruct64_t size_ds_struct64_t;
    off_t size_off_t_res;

    unsigned char shift = 0;
    
    while (shift < 64) {
    
	size_off_t = 1ULL << shift;
	size_ds_struct64_t = to_dsStruct64_t(size_off_t); /* Convert off_t to dsStruct64_t */
	size_off_t_res = to_off_t(size_ds_struct64_t);   /* Convert dsStruct64_t to off_t */
...
CuSuite* dsstruct64_off_t_get_suite()
{
    CuSuite* suite = CuSuiteNew();
    SUITE_ADD_TEST(suite, test_dsstruct64_off_t);
